...
def test_push_rejection_fetches_resets_merges_and_retries(monkeypatch, tmp_path) -> None:
    monkeypatch.chdir(tmp_path)
    calls: list[tuple[str, ...]] = []
    responses = [result("branch", "--show-current"), result("add"), result("diff", "--cached", "--quiet", code=1), result("commit"), result("push", code=1), result("fetch"), result("reset"), result("add"), result("diff", "--cached", "--quiet", code=1), result("commit"), result("push")]
    ...
    monkeypatch.setattr(git_sync, "generate_visualization", lambda: None)
    assert git_sync.commit_and_push() is True
    assert ("fetch", "origin") in calls
    assert ("reset", "--hard", "origin/work") in calls
